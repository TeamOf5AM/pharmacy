...
class POSController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        })->whereHas('batch', function ($query) use ($date) {
            $query->where('qty', '>', 0)->where('expire', '>', $date);
    ...
    public function search_product(Request $request)
    {
        ...
        $query = Medicine::where('shop_id', Auth::user()->shop_id);
        if (!empty($keyword)) {
    ...
    public function addToCart(Request $request)
    {
        ...
        $product = Medicine::with('batch')->findOrFail($productId);
        $cardData = [
            'id' => $product->id,
            'name' => $product->name,
    ...
    public function setBatch(Request $request)
    {
        ...
        if (!empty($batch)) {
            ...
            if (array_key_exists($cartId, $cart)) {
                ...
                $cart[$cartId]['price'] = number_format($batch->price, 2);
    ...
    public function place_order(Request $request)
    {
        ...
        if (session()->has('cart_store')) {
            if (count(session()->get('cart_store')) < 1) {
                $response = [
                    'error' => 1,
                    'success' => 0,
        ...
        $invocie->customer_id = $user_id;
        $invocie->date = date('Y-m-d');
        $invocie->subtotal = $request->sub_total;
        $invocie->total_price = $request->grand_total;
        $invocie->qty = $total_quantity;
        $invocie->tax = $request->tax;
        $invocie->paid_amount = $request->paid_amount;
        $invocie->due_price = $request->due_amount;
        $invocie->returned_amount = $request->change_amount;
        $invocie->discount = $request->total_discount;


        $invocie->inv_id = uniqueOrderId($offerNo, Auth::user()->shop->prefix, 'invoices', 'inv_id');
        $invocie->medicines = json_encode($medicines);

        if ($request->due_amount > 0) {
            $sup = Customer::where('id', $user_id)->first();
            if ($sup != null) {
                $sup->due += $request->due_amount;
                $sup->save();
            }
        }
        $invocie->thana_id = Auth::user()->shop->thana_id;
        $invocie->shop_id = Auth::user()->shop_id;
        $invocie->district_id = Auth::user()->shop->district_id;
        ...
        if ($invocie->save()) {
            ...
            $invpay->customer_id = $user_id;
            $invpay->method_id = $request->payment_method;
            $invpay->save();

            if (gettype($request->payment_method) != 'string') {
                $method = Method::find($request->payment_method);
                $method->balance += $request->payment_method;
                $method->save();
            }

            foreach ($items as $item) {
                $batch = Batch::where('id', $item['batch_id'])->where('medicine_id', $item['id'])->where('shop_id', Auth::user()->shop_id)->first();
                if (!empty($batch)) {
        ...
        //        $data['invoice'] = Invoice::where('shop_id', Auth::user()->shop_id)->where('id', $invocie->id)->firstOrFail();
        //        $data['transaction'] = InvoicePay::where('invoice_id', $invocie->id)->get();
        ////        PDF::loadView('invoice.print', $data)->setPaper(array(0, 0, 300, 650))->setOptions(['dpi' => 100, 'defaultFont' => 'sans-serif'])->save('public/pdf/invoice_' . $invocie->id . '.pdf');
        //        $invoice_file = 'pdf/invoice_' . $invocie->id . '.pdf';
        //        $data['file'] = $invoice_file;
        ...
        //        return redirect()->route('invoice.print', $invocie->id)->with($data);
        return response()->json($response);
    }
