...
class Helpers
{
    ...
    {
        $config = null;
        $check = ['name', 'address', 'currency', 'division_id', 'district_id', 'upazilla_id', 'union_id', 'package_id', 'next_pay', 'prefix', 'theme', 'copyright_text'];

        if (in_array($name, $check) == true && session()->has($name)) {
            $config = session($name);
        } else {
            $data = Shop::select($name)->where('shop_id', Auth::user()->shop_id)->first();
            if (isset($data)) {
    ...
    public static function saveJSONFile($code, $data)
    {
        ...
        file_put_contents(base_path('resources/lang/en/pages.json'), stripslashes($jsonData));
    }
    ...
    public static function currency_load()
    {
            ...
            session()->put('system_default_currency_info', Auth::user()->shop->currency);
            session()->put('currency_code', Auth::user()->shop->currency);
            session()->put('currency_symbol', Auth::user()->shop->currency);
    ...
    public static function language_load()
    {
        if (\session()->has('language')) {
            ...
            \session()->put('language_settings', Auth::user()->shop->language);
        }
        return Auth::user()->shop->language;
    }
